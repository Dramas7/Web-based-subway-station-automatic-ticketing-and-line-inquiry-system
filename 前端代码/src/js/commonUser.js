window.addEventListener('load', function () {
    ...
    /**@type {HTMLDivElement} -票价规则框 */
    const fareRulesVal = document.querySelectorAll('.fare-rules-val');
    // 从数据库中获取票价规则数据，渲染到fareRules栏中
    ajax({
        method: 'POST',
        url: `${prefix}/admin/base/price`,
        success: function (res) {
            if (res.code === 200) {
                fareRulesVal.forEach(function (val, index) {
                    val.innerHTML = parseInt(res.data[`price${index + 1}`]);
                });
            }
        }
    });
    ...
    function ticketPriceHandler() {
        ...
        ajax({
            ...
            success: function (res) {
                if (res.code === 200) {
                    let gradFirst = parseFloat(res.data.price1); // 起步价(<4km)固定为2元
                    let gradSecond = parseFloat(res.data.price2); // 第二梯度(4km-12km)价格为4公里/元
                    let gradThird = parseFloat(res.data.price3); // 第三梯度(12km-24km)价格为6公里/元
                    let gradForth = parseFloat(res.data.price4); // 第四梯度(>24km)价格为8公里/元
    ...
    buyTicket.addEventListener('click', function () {
        // TODO: 模拟或做一个支付功能，点击购买后弹出一个支付信息框，购买成功后则返回一个二维码
        popupErrMsg('请付款，付款成功后会返回一个二维码，请凭该二维码在地铁站中进出闸。');
        // 获取arrSelection中对应的起点站与终点站的实际名称
        let startStation = document.querySelector(`#${arrSelection[0]}`).getAttribute('number');
        let endStation = document.querySelector(`#${arrSelection[1]}`).getAttribute('number');
        /***************************************发起Ajax请求********************************/
        ajax({
            method: 'POST',
            url: `${prefix}/home/order/add`,
            data: {
                utoken: localStorage.getItem('utoken'),
                start: startStation,
                end: endStation,
                price: ticketPriceHandler()
            },
            success: (res) => {
                console.log(res);
            }
    ...
    addCollection.addEventListener('click', function () {
        // TODO: 让对应的线路数据存进用户个人的收藏栏
        // TODO: 如果线路已经存在，则弹出自制警示框已经添加了该条线路，不要重复添加，且不加进收藏栏
        // FIXME: slideDownInit()函数失效
        // 获取经由佛洛依德算法计算的最短路径信息
        let { path: stationGroup, weight: distance } = graph.getPath(...arrSelection);
        // 定义一个用于存储途经站点UID的数组
        const pass = [];
        // 获取UID
        // FIXME:无论什么情景，对于中转站，其uid都是一致的
        for (let val of stationGroup) {
            let uid = document.querySelector(`#${val}`).getAttribute('number');
            pass.push(uid);
        }
        // 先执行一次timeHandler,获取transferCount
        timeHandler();
        // 获取arrSelection中对应的起点站与终点站的实际名称
        let startStation = document.querySelector(`#${arrSelection[0]}`).getAttribute('text');
        let endStation = document.querySelector(`#${arrSelection[1]}`).getAttribute('text');
        // 换乘次数由先前定义的外部变量获得
        // 途经站点个数计算
        let stationCount = stationGroup.length;
        console.log(pass);

        /******************************************* 发起ajax请求 *********************************/
        ajax({
            method: 'POST',
            url: `${prefix}/home/cart/add`,
            data: {
                utoken: localStorage.getItem('utoken'),
                start: pass[0],
                end: pass[pass.length - 1],
                distance: distance,
                price: ticketPriceHandler(),
                pass: pass,
                change_time: transferCount,
                site_num: stationCount,
                estimate_time: `${timeHandler()}`
            },
            success: (res) => {
                if (res.code === 200) {
                    console.log('添加收藏成功');
        ...
        /******************************************* 发起ajax请求 *********************************/
        // 弹出待确认消息
        popupErrMsg('成功添加该条线路，您可以点击页面顶部的“线路收藏”按钮查看。');
    });
    ...
    ajax({
        ...
        success: (res) => {
            ...
            for (let i = 0; i < lineUnits.length; i++) {
                lineUnits[i].setAttribute('distance', data[i].distance);
            }
        }
    });
    ...
    /*************************************线路收藏及查看订单栏 **************************************/
    // 获取查看订单按钮
    const checkOrderBtn = document.querySelector('body > header > nav > a:nth-child(1)');
    // 获取线路收藏按钮
    const collectLineBtn = document.querySelector('body > header > nav > a:nth-child(2)');
    // 获取查看订单弹出框
    const orderCheckModalBox = document.querySelector('#order-check-modal-box');
    // 获取线路收藏弹出框
    const lineCollectModalBox = document.querySelector('#line-collect-modal-box');
    // 获取查看订单的关闭按钮
    const orderCloseBtn = document.querySelector('#order-check-modal-box #close-btn');
    // 获取线路收藏的关闭按钮
    const collectCloseBtn = document.querySelector('#line-collect-modal-box #close-btn');

    // 为查看订单按钮绑定CLICK事件
    checkOrderBtn.addEventListener('click', function () {
        orderCheckModalBox.style.display = 'block';
        maskLayer.style.display = 'block';
        // 获取查看订单列表
        const tbodyList = orderCheckModalBox.children[0].children[1];
        // 先清空tbodyList的列表
        tbodyList.innerHTML = '';
        ...
        ajax({
            ...
            success: (res) => {
                ...
                for (let i = 0; i < res.data.length; i++) {
                    ...
                    tr.innerHTML = `
                        <td><span>${res.data[i].start}</span><object data="../svg/arrowRight.svg"></object><span>${res.data[i].end}</span></td>
                        <td>
                            <div id="ticket-price"><span>${res.data[i].price}</span><span>¥</span></div>
                        </td>
                        <td>${res.data[i].create_time}</td>
                        <td>
                            <button class="apply-for-refund">申请退票</button>
                ...
                // FIXME:为何将事件处理函数定义在外部，而将该函数以函数名的形式放在内部作用域中却无法访问内部的一些变量？
                // 定义refundHandler事件处理函数
                // function refundHandler(e) {}
                console.log(res);
                // 获取申请退票按钮
                const applyForRefund = document.querySelectorAll('.apply-for-refund');
                // 刷新查看订单界面时，通过res.status属性来判断当前的申请状态，从而改变订单栏的状态
                applyForRefund.forEach(function (val, index) {
                    if (res.data[index].status == '退票申请中') {
                        val.disabled = true;
                        val.style.cursor = 'default';
                        val.style.backgroundColor = '#ccc';
                        val.textContent = '正在申请';
                    } else if (res.data[index].status == '退票成功') {
                        val.disabled = true;
                        val.style.cursor = 'default';
                        val.style.backgroundColor = '#ccc';
                        val.textContent = '已经退款';
                        val.style.color = 'green';
                    } else if (res.data[index].status == '退票失败') {
                        val.disabled = true;
                        val.style.cursor = 'default';
                        val.style.backgroundColor = '#ccc';
                        val.textContent = '未有退款';
                        val.style.color = 'red';
                    }
                });
                // 为所有申请退票按钮添加CLICK事件
                for (let i = 0; i < applyForRefund.length; i++) {
                    applyForRefund[i].addEventListener('click', function (e) {
                        e.stopPropagation();
                        // 获取弹出模态框
                        const refundInfoModalBox = document.querySelector('.input-refund-info');
                        // 获取模态框中的提交与取消提交按钮
                        const submit = refundInfoModalBox.children[0].children[2];
                        const cancel = refundInfoModalBox.children[0].children[3];
                        // 获取信息输入框
                        const refundReasonIpt = document.querySelector('#refund-reason');
                        // 让弹出的模态框展示
                        refundInfoModalBox.style.display = 'block';
                        // 定义提交按钮的事件处理函数
                        function submitHandler(e) {
                            e.stopPropagation();
                            // 判断字数是否达标
                            if (this.previousElementSibling.value.length < 3) {
                                popupErrMsg('您输入的字数过少，请输入3~10个字');
                            } else if (this.previousElementSibling.value.length > 10) {
                                popupErrMsg('您输入的字数过多，请输入3~10个字');
                            } else {
                                /************************************发起Ajax请求 ******************************/
                                ajax({
                                    method: 'POST',
                                    url: `${prefix}/home/order/edit`,
                                    data: {
                                        utoken: localStorage.getItem('utoken'),
                                        id: res.data[i].id,
                                        reason: refundReasonIpt.value
                                    },
                                    success: (res) => {
                                        applyForRefund[i].disabled = true;
                                        applyForRefund[i].style.backgroundColor = '#ccc';
                                        applyForRefund[i].textContent = '正在申请';
                                        applyForRefund[i].style.cursor = 'default';
                                        console.log(res);
                                    }
                                });
                                /************************************发起Ajax请求 ******************************/
                                popupErrMsg('提交申请成功！请耐心等待管理员审批');
                            }
                            this.parentNode.parentNode.style.display = 'none';
                            // 让input栏内容清空
                ...
                for (let i = 0; i < deleteOrder.length; i++) {
                    deleteOrder[i].addEventListener('click', function (e) {
                        e.stopPropagation();
                        // 获取提示是否删除的信息框
                        const tipsBox = document.querySelector('.deleted-order-tip');
                        // 获取是和否的按钮
                        const yes = tipsBox.children[0].children[2];
                        const no = tipsBox.children[0].children[3];
                        //获取起始站与终点站的文本信息
                        const start = this.parentNode.parentNode.children[0].children[0];
                        const end = this.parentNode.parentNode.children[0].children[2];
                        // 在点击删除订单按钮时，将提示框展示，并将起点与终点的文本信息输进去
                        tipsBox.children[0].children[1].children[0].innerHTML = start.innerHTML;
                        tipsBox.children[0].children[1].children[1].innerHTML = end.innerHTML;
    ...
    // 为线路收藏按钮添加CLICK事件
    collectLineBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        lineCollectModalBox.style.display = 'block';
        maskLayer.style.display = 'block';
        /***********************************发起ajax请求获取收藏列表 *********************/
        ajax({
            method: 'POST',
            url: `${prefix}/home/cart/index`,
            data: {
                utoken: localStorage.getItem('utoken')
            },
            success: function (res) {
                ...
                if (res.code === 200) {
                    // 获取线路收藏栏的tbodyList
                    const tbodyList = lineCollectModalBox.children[0].children[1];
                    // 让tbodyList先清空
                    tbodyList.innerHTML = '';
                    for (let i = 0; i < res.data.length; i++) {
                        const tr = document.createElement('tr');
                        tr.innerHTML = `
                            <td><span>${res.data[i].start}</span><object data="../svg/arrowRight.svg"></object><span>${res.data[i].end}</span></td>
                            <td>${res.data[i].change_time}次</td>
                            <td>${res.data[i].site_num}个</td>
                            <td>
                                <div id="station-list" class="list-wrapper line-collect">
                                    <img src="../svg/slidedown.svg" class="slide-down-icon">
                                    <!-- 列表标题 -->
                                    <div class="title"><span class="color-light-green">-</span>途经站点<span class="color-light-green">-&gt;</span></div>
                                    <!-- 用于填充标题与列表之间，仅做填充空间用 -->
                                    <div class="fill" style="display: none;"></div>
                                    <ul class="list" style="height: 0px; display: none; overflow: hidden;"></ul>
                                </div>
                            </td>
                            <td>${parseFloat(res.data[i].estimate_time).toFixed(1)}min</td>
                            <td>
                                <div id="ticket-price"><span>${parseFloat(res.data[i].price).toFixed(1)}</span><span>¥</span></div>
                            </td>
                            <td>
                                <button id="delete-collect">删除收藏</button>
                            </td>
                        `;
                        // 从tr中获取下拉菜单主体
                        const list = tr.querySelector('#station-list').querySelector('.list');
                        for (let j = 0; j < res.data[i].pass.length; j++) {
                            const li = document.createElement('li');
                            let lineId = document.querySelector(`*[text=${res.data[i].pass[j]}]`).parentNode.id;
                            // 对常规线路进行筛选
                            if (lineId == 'red_line_stations') {
                                li.innerHTML = `
                                <span class="red-line line-icon">1</span>${res.data[i].pass[j]}
                            `;
                            } else if (lineId == 'blue_line_stations') {
                                li.innerHTML = `
                                <span class="blue-line line-icon">2</span>${res.data[i].pass[j]}
                            `;
                            } else if (lineId == 'purple_line_stations') {
                                li.innerHTML = `
                                <span class="purple-line line-icon">3</span>${res.data[i].pass[j]}
                            `;
                            } else if (lineId == 'green_line_stations') {
                                li.innerHTML = `
                                <span class="green-line line-icon">4</span>${res.data[i].pass[j]}
                            `;
                            }

                            // 进一步对中转站筛选
                            switch (res.data[i].pass[j]) {
                                case '江门站':
                                    li.innerHTML = `
                                    <span class="red-line line-icon">1</span><span class="blue-line line-icon">2</span>${res.data[i].pass[j]}
                                `;
                                    break;
                                case '中心南路':
                                    li.innerHTML = `
                                    <span class="red-line line-icon">1</span><span class="purple-line line-icon">3</span>${res.data[i].pass[j]}
                                `;
                                    break;
                                case '市政府':
                                    li.innerHTML = `
                                ...
                                case '龙溪':
                                    li.innerHTML = `
                                    <span class="red-line line-icon">1</span><span class="green-line line-icon">4</span>${res.data[i].pass[j]}
                                `;
                                    break;
                                case '滨江':
                                    li.innerHTML = `
                                ...
                                case '东海路':
                                    li.innerHTML = `
                                    <span class="blue-line line-icon">2</span><span class="purple-line line-icon">3</span>${res.data[i].pass[j]}
                                `;
                                    break;
                                case '东方广场':
                                    li.innerHTML = `
                                    <span class="blue-line line-icon">2</span><span class="green-line line-icon">4</span>${res.data[i].pass[j]}
                                `;
                            }
                        ...
                        tbodyList.appendChild(tr);
                        // 获取删除收藏按钮
                        const deleteCollect = document.querySelectorAll('#delete-collect');
                        // 为删除收藏按钮绑定事件
                        deleteCollect[i].addEventListener('click', function (e) {
                            e.stopPropagation();
                            /***********************发起ajax请求 **************************/
                            ajax({
                                method: 'POST',
                                url: `${prefix}/home/cart/delete`,
                                data: {
                                    utoken: localStorage.getItem('utoken'),
                                    id: res.data[i].id
                                },
                    ...
                    // 重新获取下拉菜单信息栏
                    // 下拉菜单展开按钮
                    let slideDownButton = tbodyList.querySelectorAll('.slide-down-icon');
                    // 下拉菜单标题
                    let slideTitle = tbodyList.querySelectorAll('.list-wrapper .title');
                    // 下拉菜单主体
                    let slideList = tbodyList.querySelectorAll('.list-wrapper .list');
                    // 下来菜单主体与标题之间的填充区
                    let fillArea = tbodyList.querySelectorAll('.list-wrapper .fill');
                    console.log(slideDownButton);
                    for (let val of slideDownButton) {
                        val.addEventListener('click', handleSlideDown);
                    }
                    initSlideDown();
                } else {
    ...
    // 为线路收藏的关闭按钮添加CLICK事件
    collectCloseBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        this.parentNode.style.display = 'none';
        maskLayer.style.display = 'none';
        // 重新获取下拉菜单内容，让slideDownInit()函数生效内容生效
        initSlideDown();
    });

    // 设计代码将数据自动输入数据库
    /***************************************用于设置各个站点的自定义number属性*************************/
    // 定义各个地铁站点
    let subwayStation = {
        firstLineStations: {
            ele: redLineStations,
        ...
        secondLineStations: {
            ele: blueLineStations,
            num: 'B',
            line: 2
        },
        thirdLineStations: {
            ele: purpleLineStations,
            num: 'C',
            line: 3
        },
        forthLineStations: {
            ele: greenLineStations,
            num: 'D',
            line: 4
        }
    };
    ...
    function setNum(stations, aph) {
        stations.forEach(function (val, index) {
            val.setAttribute('number', `${aph}${padZero(index + 1)}`);
        });
    }
    // 定义补零函数
    ...
    for (let k in subwayStation) {
        subwayStation[k].ele.forEach(function (val) {
            val.setAttribute('line', subwayStation[k].line);
        });
    }

    // const stationInfo = [];
    // const sumStation = [...redLineStations, ...blueLineStations, ...purpleLineStations, ...greenLineStations];

    // sumStation.forEach(function (val) {
    //     const obj = {};
    //     obj.name = val.getAttribute('text');
    //     obj.uid = val.getAttribute('number');
    //     obj.line = val.getAttribute('line');
    ...
    // ajax({
    //     method: 'POST',
    //     url: 'http://test3.jsnaili.wang/admin/site/delete',
    //     data: {
