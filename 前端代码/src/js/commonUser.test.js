window.addEventListener('load', function () {
    ...
    ajax({
        method: 'POST',
        url: 'http://test3.jsnaili.wang/admin/base/price',
    ...
    buyTicket.addEventListener('click', function () {
        ...
        tr.innerHTML = `
            <td><span>${startStation}</span><object data="../svg/arrowRight.svg"></object><span>${endStation}</span></td>
            <td>
                <div id="ticket-price"><span>${ticketPriceHandler()}</span><span>¥</span></div>
            </td>
            <td>${dateFormat()}</td>
            <td>
        ...
        // 为最后一个申请退票按钮添加CLICK事件
        applyForRefund[applyForRefund.length - 1].addEventListener('click', refundHandler);
        // 获取删除订单按钮
        const deleteOrder = document.querySelectorAll('.delete-order');
        // 为删除订单按钮绑定CLICK事件
        deleteOrder[deleteOrder.length - 1].addEventListener('click', function (e) {
            e.stopPropagation();
            // 获取提示是否删除的信息框
    ...
    addCollection.addEventListener('click', function () {
        ...
        tr.innerHTML = `
                        <td><span>${startStation}</span><object data="../svg/arrowRight.svg"></object><span>${endStation}</span></td>
                        <td>${transferCount}次</td>
                        <td>${stationCount}个</td>
                        <td>
        ...
        // 重新获取下拉菜单信息栏
        // 下拉菜单展开按钮
        slideDownButton = document.querySelectorAll('.list-wrapper .slide-down-icon');
        // 下拉菜单标题
        slideTitle = document.querySelectorAll('.list-wrapper .title');
        // 下拉菜单主体
        slideList = document.querySelectorAll('.list-wrapper .list');
        // 下来菜单主体与标题之间的填充区
        fillArea = document.querySelectorAll('.list-wrapper .fill');
        // 获取删除收藏按钮
        const deleteCollect = document.querySelectorAll('#delete-collect');
        // 为最后一个删除收藏按钮绑定事件
        deleteCollect[deleteCollect.length - 1].addEventListener('click', function (e) {
            e.stopPropagation();
            this.parentNode.parentNode.remove();
        });
        // 为最后一个下拉按钮添加点击事件
        slideDownButton[slideDownButton.length - 1].addEventListener('click', handleSlideDown);
        // 执行stationList()函数，添加途经站点
        stationListHandler('line-collect');
        // 弹出待确认消息
    ...
    //         val.setAttribute('number', `${aph}${index + 1}`);
    //     });
    // }
    // // 调用number函数，遍历每一个站点
